...
def collection_user_list():
  col_usr_dbs, col_usr_cursor = model.CollectionUser.get_dbs(collection=util.param('collection',ndb.Key))
...
# User Update
###############################################################################
class CollectionUserUpdateForm(wtf.Form):
...
@auth.admin_required
def collection_user_update(col_key=None):
  update = False
  col_db = True
  if col_key:
