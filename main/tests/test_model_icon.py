...
class TestIcon(unittest.TestCase):
  ...
  def test_init_with_type_data_and_collection(self):
    ...
    i3 = model.Icon(icon=self.i3s_data,name='i3s_data',collection='one')
    ndb.put_multi([i2,i3])
  ...
  def test_init_icon_with_collection(self):
    i1 = model.Icon(icon=self.i1s_name,name='i1s_name', collection='one')
    i1.put()
    assert i1 is not None
    self.assertEqual(i1.name, 'i1s_name')
  ...
  def test_add_icon(self):
    ...
    # this collection
    key2 = model.Icon.add(key,collection='test1')
    icon_db = key.get()
    icon2_db = key2.get()
    self.assertEqual(icon_db.count,3)
    self.assertEqual(icon2_db.count,1)
    ...
    key3 = model.Icon.add(key,collection='test1')
    self.assertEqual(key2,key3)
    icon2_db = key2.get()
    self.assertEqual(icon2_db.count,2)
    # and a third time the same
    key4 = model.Icon.add(key,collection='test1')
    ...
    keyA = model.Icon.add(key2,collection='test2')
    icon_db = key.get()
    icon2_db = key2.get()
    iconA_db = keyA.get()
    ...
    key2a = model.Icon.add(key2,collection='test1a',as_child=True)
    icon_db = key.get()
    icon2_db = key2.get()
    icon2a_db = key2a.get()
  ...
  def test_get_icon_by_toplevel(self):
    key = model.Icon.create(icon=self.i1s_name,name='i')
    for i in range(0,10):
      key2 = model.Icon.add(key,collection='test2')
      key3 = model.Icon.add(key,collection='test3')
      key3a = model.Icon.add(key3,collection='test3a',as_child=True)
      key10 = model.Icon.add(key,collection='test{}'.format(i+10))
    icon_db = key.get()
    icon2_db = key2.get()
    icon3_db = key3.get()
    icon3a_db = key3a.get()
    ...
    top_dbs = model.Icon.get_by_toplevel(collection='global')
    self.assertEqual(len(top_dbs),2)
    # test with collections
    test2_dbs = model.Icon.get_by_toplevel(key,collection='test2')
    test12_dbs = model.Icon.get_by_toplevel(key,collection='test12')
...
class TestIconizedModel(unittest.TestCase):
  ...
  def setUp(self):
    ...
    class TestIconModel(model.Iconize, ndb.Model):
  ...
  def test_iconized_with_collection(self):
    ...
    icon_dbs = model.Icon.qry(collection='one').fetch()
    self.assertEqual(len(icon_dbs),1)
